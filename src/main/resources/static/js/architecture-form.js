...
descriptionInput.on('keyup', () => {
    characterCounter.text(`${400 - descriptionInput.val().length} ${getMessage('info.characters-left')}`);
});
...
function deleteFile() {
    ...
    fileInputDiv.html('<div class="input-group custom-file-button">' +
        `<label class="input-group-text bg-light w-25" for="fileInput">${getMessage('logo')}</label>` +
