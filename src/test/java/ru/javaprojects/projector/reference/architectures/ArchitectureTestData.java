...
public class ArchitectureTestData {
    ...
    public static final MockMultipartFile LOGO_FILE = new MockMultipartFile("logo.inputtedFile", "Event driven.png",
    ...
    public static ArchitectureTo getNewTo() {
        return new ArchitectureTo(null, "Event Driven", "Event Driven architecture description", LOGO_FILE);
    }

    public static Architecture getNew(String contentPath) {
        ArchitectureTo newTo = getNewTo();
        return new Architecture(null, newTo.getName(), newTo.getDescription(), new File("event_driven.png",
                contentPath + "event_driven" + "/" + "event_driven.png"));
    }

    public static Architecture getUpdated(String contentPath) {
        ...
        return new Architecture(ARCHITECTURE1_ID, updatedName, "updated architecture description",
                new File(UPDATED_LOGO_FILE.getOriginalFilename(), contentPath +
                        updatedName.toLowerCase().replace(" ", "_") + "/" + UPDATED_LOGO_FILE.getOriginalFilename()));
    }

    public static Architecture getUpdatedWhenOldLogo(String contentPath) {
        String updatedName = "updated architecture name";
        return new Architecture(ARCHITECTURE1_ID, updatedName, "updated architecture description",
                new File(architecture1.getLogo().getFileName(), contentPath + updatedName.toLowerCase().replace(" ", "_") + "/" +
                        architecture1.getLogo().getFileName()));
    }

    public static Architecture getUpdatedWhenOldName(String contentPath) {
        return new Architecture(ARCHITECTURE1_ID, architecture1.getName(), "updated architecture description",
                new File(UPDATED_LOGO_FILE.getOriginalFilename(), contentPath + architecture1.getName().toLowerCase().replace(" ", "_") + "/" +
    ...
    public static MultiValueMap<String, String> getUpdatedParams(String contentPath) {
        MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
        Architecture updated = getUpdated(contentPath);
